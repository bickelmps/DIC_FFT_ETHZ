...
if geotiff == 0
    cd Input
    master = imread('BRIP2003.tif'); % used for DIC
    orig_m = imread('BRIP2003.tif'); % used for plotting
    % Slave image
    slave = imread('BRIP2006.tif');
    orig_s = imread('BRIP2006.tif');
    cd ..
end
if geotiff == 1
    cd Input
    inputfilename = 'BRIP2003';
    outfilename = 'DIC_2003-2006';
    master = imread('BRIP2003.tif'); % used for DIC
    orig_m = imread('BRIP2003.tif'); % used for plotting
    % Slave image
    slave = imread('BRIP2006.tif');
...
% Modified by V. Bickel, ETH Zurich
if geotiff == 1
    [a,b] = size(J);
    if filter_selection == 1
        pp(pp == 0) = 1; % vector correction for reshape, RMSE filter is not forwarded for geotiff
