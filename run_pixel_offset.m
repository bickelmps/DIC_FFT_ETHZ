...
pix = 0.25; % Pixel size or GSD (in meters), only required if geotiff = 0
        ...
        cd Input
        % Master image
[master, R] = geotiffread('YOURIMAGEHERE.tif'); % read
        % Slave image
[slave, ~]  = geotiffread('YOURIMAGEHERE.tif'); % read
        cd ..
...
%% Co-Registration
        if coreg == 1
         imagesc(master(:,:,1)); colormap gray; title("NOTE: if bbox size is too small, coreg might fail")
        ...
        tic
        [T0,T1] = co_registration(master,slave,sp,co_os,coreg,rect);        
        toc  
...
%% Forwarding Geotiff information
        ...
        if filter_selection == 3
         ...
         %close
         Q2 = imresize(outfigure,size(master(:,:,1)),'near');
...
% Multi-band reduction of input images, if required
        [x, y, z] = size(master);
        if z > 3
            orig_m_int(:,:,1) = master(:,:,1);
            orig_m_int(:,:,2) = master(:,:,2);
            orig_m_int(:,:,3) = master(:,:,3);
            orig_m = orig_m_int;
        
            orig_s_int(:,:,1) = slave(:,:,1);
            orig_s_int(:,:,2) = slave(:,:,2);
            orig_s_int(:,:,3) = slave(:,:,3);
            orig_s = orig_s_int;
        else
            orig_m = master;
            orig_s = slave;
        end
        clear master slave orig_m_int orig_s_int
